...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, criterion, metric_ftns, optimizer, config, data_loader,
                 valid_data_loader=None, lr_scheduler=None, len_epoch=None, distributed=False, pretrained=None,rank=0
                 ,add_bt=0):
        super().__init__(model, criterion, metric_ftns, optimizer, config, distributed, gpu=rank)
        self.config = config
        self.rank = rank
        self.add_bt = add_bt
        if self.add_bt:
        ...
        if self.distill:
            ...
            pretrained_checkpoint = torch.load(pretrained, map_location="cpu")
            load_state_dict(self.model, pretrained_checkpoint['state_dict'], no_ignore=True)
            pass
        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            with autocast():
                if self.real_model.requires_target:
                    ...
                    if self.distill:
                        with torch.no_grad():
                            teacher = self.teacher_model(data, target=target)
    ...
    def cal_loss(self, extra_info, loss, output, target, teacher):
