...
def soft_label_loss(outputs, targets, weight=None):
    log_softmax = F.log_softmax(outputs, dim=1)


    # 2.1 same as convention
    loss = - torch.sum(log_softmax * targets) / len(log_softmax)
    return loss
...
class RIDELoss(nn.Module):
    ...
    def forward(self, output_logits, target, extra_info=None):
        ...
        for logits_item in extra_info['logits']:
            ...
            if self.m_list is None:
                if 'soft_target' in extra_info:
                    loss += self.base_loss_factor * soft_label_loss(ride_loss_logits, target if 'soft_target' not in extra_info else extra_info['soft_target'])
                else:
                ...
                if 'soft_target' in extra_info:
                    loss += self.base_loss_factor * soft_label_loss(final_output, target if 'soft_target' not in extra_info else extra_info['soft_target'], weight=self.per_cls_weights_base)
                else:
