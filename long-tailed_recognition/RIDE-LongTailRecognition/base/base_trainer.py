...
class BaseTrainer:
    ...
    """
    def __init__(self, model, criterion, metric_ftns, optimizer, config, distributed=False, gpu=0):
        self.config = config
        ...
        if distributed:
            print('distributed---', self.device_ids)
    ...
    def train_dist(self, args, ngpus_per_node):
        """
        Full training logic
        """
        not_improved_count = 0
    ...
    def _save_checkpoint(self, epoch, save_best=False, best_only=False, checkpoint=False):
        """
        Saving checkpoints
        ...
        if not best_only:
            ...
            import os
            pre_filename = str(self.checkpoint_dir / 'checkpoint-epoch{}.pth'.format(epoch - self.save_period))
