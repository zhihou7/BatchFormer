...
def update(config, args):
    ...
    if args.lr_scale != 1.:
        print('before lr', config['networks']['feat_model']['optim_params']['lr'])
        config['networks']['classifier']['optim_params']['lr'] *= args.lr_scale
        config['networks']['feat_model']['optim_params']['lr'] *= args.lr_scale
        print('after lr', config['networks']['feat_model']['optim_params']['lr'])
...
if args.add_bt:
    if args.add_bt:
        config['training_opt']['log_dir'] = config['training_opt']['log_dir'] + '_'+ 'rebuttal_'+ str(args.add_bt)+'mlp'+str(args.mlp)
...
if not test_mode:
    ...
    if sampler_defs and sampler_defs['type'] == 'MetaSampler':   # todo: use meta-sampler
        ...
        data['meta'] = dataloader.load_data(data_root=data_root[dataset.rstrip('_LT')],
                                    ...
                                    cifar_imb_ratio=training_opt['cifar_imb_ratio'] if 'cifar_imb_ratio' in training_opt else None,
                                    meta=True)
        training_model = model(config, data, test=False, meta_sample=True, learner=learner, add_bt=args.add_bt, debug=args.debug)
    else:
        training_model = model(config, data, test=False, add_bt=args.add_bt, debug=args.debug)
    ...
    training_model = model(config, data, test=True, add_bt=args.add_bt, eval_batch=args.eval_batch, debug=args.debug)
