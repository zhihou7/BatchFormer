...
class ERM(Algorithm):
    ...
    def update(self, x, y, **kwargs):
        all_x = torch.cat(x)
        all_y = torch.cat(y)
        all_preds = self.predict(all_x)
        while len(all_preds) > len(all_y):
            all_y = torch.cat([all_y, all_y], dim=0)
        loss = F.cross_entropy(all_preds, all_y)
...
class Mixstyle2(Algorithm):
    ...
    def update(self, x, y, **kwargs):
        ...
        for (xi, yi), (xj, yj) in pairs:
            ...
            x2 = torch.cat([xi, xj])
            y2 = torch.cat([yi, yj])
            preds = self.predict(x2)
            while len(preds)> len(y2):
                y2 = torch.cat([y2, y2], dim=0)
            loss += F.cross_entropy(preds, y2)
...
class AbstractDANN(Algorithm):
    ...
    def update(self, x, y, **kwargs):
        ...
        if len(disc_out) > len(disc_labels):
            disc_labels = torch.cat([disc_labels, disc_labels], dim=0)
