...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
    ...
    for i in range(args.iters_per_epoch):
        ...
        if len(outputs) > len(x):
            y_s, y_t, y_s1, y_t1 = outputs.chunk(4, dim=0)
            y_s = torch.cat([y_s, y_s1], dim=0)
            y_t = torch.cat([y_t, y_t1], dim=0)
            y_s_adv, y_t_adv, y_s_adv1, y_t_adv1 = outputs_adv.chunk(4, dim=0)
            y_s_adv = torch.cat([y_s_adv, y_s_adv1], dim=0)
            y_t_adv = torch.cat([y_t_adv, y_t_adv1], dim=0)
        else:
            y_s, y_t = outputs.chunk(2, dim=0)
            y_s_adv, y_t_adv = outputs_adv.chunk(2, dim=0)
        if len(y_s) > len(labels_s):
            labels_s = torch.cat([labels_s, labels_s], dim=0)
        if len(y_t) > len(labels_t):
