...
def train(train_iter: ForeverDataIterator, model: Classifier, optimizer, lr_scheduler: CosineAnnealingLR,
    ...
    for i in range(args.iters_per_epoch):
        ...
        if len(y_all) > len(labels_all):
            all_shape = y_all.shape
            y_all = torch.unsqueeze(y_all, 1)
            y_all = torch.cat([y_all[:len(y_all)//2], y_all[len(y_all)//2:]], dim=1)
            y_all = torch.reshape(y_all, all_shape)
            # old_label_shape = labels_all.shape
            labels_all = torch.unsqueeze(labels_all, 1)
            labels_all = torch.cat([labels_all, labels_all], dim=1)
