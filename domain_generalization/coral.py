...
class TransformerDecorator(torch.nn.Module):
    def __init__(self, backbone=None, add_bt=3, dim=2048, eval_global=0):
...
def main(args: argparse.Namespace):
    ...
    pool_layer = TransformerDecorator(pool_layer, add_bt=args.add_bt, dim=backbone.out_features)
...
def train(train_iter: ForeverDataIterator, model: Classifier, optimizer, lr_scheduler: CosineAnnealingLR,
    ...
    for i in range(args.iters_per_epoch):
        ...
        if len(y_all) > len(labels_all):
            labels_all = torch.cat([labels_all, labels_all], dim=0)
            y_all_ = y_all.chunk(n_domains_per_batch*2, dim=0)
            f_all_ = f_all.chunk(n_domains_per_batch*2, dim=0)
            labels_all_ = labels_all.chunk(n_domains_per_batch*2, dim=0)
            y_all = []
            f_all = []
            labels_all = []
            for i in range(n_domains_per_batch):
                y_all.append(torch.cat([y_all_[i], y_all_[i+n_domains_per_batch]], dim=0))
                f_all.append(torch.cat([f_all_[i], f_all_[i+n_domains_per_batch]], dim=0))
                labels_all.append(torch.cat([labels_all_[i], labels_all_[i+n_domains_per_batch]], dim=0))
        else:
